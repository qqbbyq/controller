...
/**
 * Another utility serialization/deserialization for {@link DataTreeCandidate}.
 * Note that this utility maintain before-image information across serialization.
 * Different from the DataTreeCandidateInputOutPut.
 *
 * @author Robert Varga
 * modified by @author zhuyuqing
 *
 */
@Beta
public final class CandidateInputOutput {
  private static final Logger LOG = LoggerFactory.getLogger(org.opendaylight.controller.cluster.datastore.persisted.DataTreeCandidateInputOutput.class);
  ...
  //status added by zhuyuqing  about if there are dataBefore and dataAfter
  private static final byte BOTH_BEFORE_AND_AFTER = 0;
  private static final byte BEFORE_BUT_NO_AFTER = 1;
  private static final byte AFTER_BUT_NO_BEFORE = 2;
  private static final byte NEITHER_BEFORE_AND_AFTER = 3;

  private CandidateInputOutput() {
  ...
  private static DataTreeCandidateNode readModifiedNode(final ModificationType type,
    ...
    final YangInstanceIdentifier.PathArgument identifier = in.readPathArgument();
  ...
  public static DataTreeCandidate readCandidate(final DataInput in) throws IOException {
    final NormalizedNodeDataInput reader = new NormalizedNodeInputStreamReader(in);
    final YangInstanceIdentifier rootPath = reader.readYangInstanceIdentifier();
    final byte type = reader.readByte();
 ...
 /*
  ...
  //import function which is used to transform from candidate to byte[] with getDataBefore
  public static void writeCandidate(final DataOutput out, DataTreeCandidate candidate) throws IOException {
    try (final NormalizedNodeDataOutput writer = NormalizedNodeInputOutput.newDataOutput(out)) {
      writer.writeYangInstanceIdentifier(candidate.getRootPath());
      ...
      boolean isBeforeExist = node.getDataBefore().isPresent();
      boolean isAfterExist = node.getDataAfter().isPresent();
      if(isBeforeExist && isAfterExist){
        writer.write(BOTH_BEFORE_AND_AFTER);
        writer.writeNormalizedNode(node.getDataBefore().get());
        writer.writeNormalizedNode(node.getDataAfter().get());
      } else if(isBeforeExist){
        writer.write(BEFORE_BUT_NO_AFTER);
        writer.writeNormalizedNode(node.getDataBefore().get());
      } else if(isAfterExist){
        writer.write(AFTER_BUT_NO_BEFORE);
        writer.writeNormalizedNode(node.getDataAfter().get());
      } else {
        writer.write(NEITHER_BEFORE_AND_AFTER);
      }
  ...
  private static void writeChildren(final NormalizedNodeDataOutput out,
    ...
    for (DataTreeCandidateNode child : children) {
      writeCandidateNode(out, child);
    }
  }

  private static void writeCandidateNode(final NormalizedNodeDataOutput out, final DataTreeCandidateNode node)
    throws IOException {
    switch (node.getModificationType()) {
      case APPEARED:
        out.writeByte(APPEARED);
    ...
    boolean isAfterExist = node.getDataAfter().isPresent();
    if(isBeforeExist && isAfterExist){
      out.write(BOTH_BEFORE_AND_AFTER);
      out.writeNormalizedNode(node.getDataBefore().get());
      out.writeNormalizedNode(node.getDataAfter().get());
    } else if(isBeforeExist){
      out.write(BEFORE_BUT_NO_AFTER);
      out.writeNormalizedNode(node.getDataBefore().get());
    } else if(isAfterExist){
      out.write(AFTER_BUT_NO_BEFORE);
      out.writeNormalizedNode(node.getDataAfter().get());
    } else {
      out.write(NEITHER_BEFORE_AND_AFTER);
    }

...
import org.cmcc.aero.impl.ofRpcs.impl.OFFwdServiceImpl;
...
public class GlobalFlowRpcServiceImpl implements GlobalFlowRpcService {
    ...
    public GlobalFlowRpcServiceImpl(OFFwdServiceImpl openflowFwdingSvc){
        this.delegateSvc =openflowFwdingSvc;
        this.globalRpcClient = GlobalRpcClient.getInstance();
        initService();
    }
    ...
    private String locateOFNode(InstanceIdentifier<FlowCapableNode> nodeIdent){
    ...
    public Future<GlobalRpcResult> removeFlowsBatch(RemoveFlowsBatchInput input) {
        InstanceIdentifier<FlowCapableNode> nodeIdent = (InstanceIdentifier<FlowCapableNode>) input.getNode().getValue();
        String resourecePath = locateOFNode(nodeIdent);
